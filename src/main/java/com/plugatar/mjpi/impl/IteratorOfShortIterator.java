...
/**
 * The {@link Iterator} implementation based on {@link ShortIterator}.
 *
 * @param <T> the type of Iterator items
 */
public final class IteratorOfShortIterator<T> implements Iterator<T> {
    private final ShortIterator origin;
    private final ShortFunction<? extends T> conversionFunction;

    /**
     * Ctor.
     *
     * @param origin             the origin iterator
     * @param conversionFunction the function to convert {@code short}-valued items to type {@code T}
     * @throws NullPointerException if {@code origin} or {@code conversionFunction} is null
     */
    public IteratorOfShortIterator(final ShortIterator origin,
                                   final ShortFunction<? extends T> conversionFunction) {
