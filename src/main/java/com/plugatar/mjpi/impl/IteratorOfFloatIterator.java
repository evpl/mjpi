...
/**
 * The {@link Iterator} implementation based on {@link FloatIterator}.
 *
 * @param <T> the type of Iterator items
 */
public final class IteratorOfFloatIterator<T> implements Iterator<T> {
    private final FloatIterator origin;
    private final FloatFunction<? extends T> conversionFunction;

    /**
     * Ctor.
     *
     * @param origin             the origin iterator
     * @param conversionFunction the function to convert {@code float}-valued items to type {@code T}
     * @throws NullPointerException if {@code origin} or {@code conversionFunction} is null
     */
    public IteratorOfFloatIterator(final FloatIterator origin,
                                   final FloatFunction<? extends T> conversionFunction) {
