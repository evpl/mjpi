...
/**
 * The {@link ShortIterator} implementation based on {@link Iterator}.
 *
 * @param <T> the type of wrapped Iterator items
 */
public final class ShortIteratorOfIterator<T> implements ShortIterator {
    private final Iterator<? extends T> origin;
    private final ToShortFunction<? super T> conversionFunction;

    /**
     * Ctor.
     *
     * @param origin             the origin iterator
     * @param conversionFunction the function to convert items of type {@code T} to {@code short}
     * @throws NullPointerException if {@code origin} or {@code conversionFunction} is null
     */
    public ShortIteratorOfIterator(final Iterator<? extends T> origin,
                                   final ToShortFunction<? super T> conversionFunction) {
    ...
    @Override
    public short next() {
        return this.conversionFunction.applyAsShort(this.origin.next());
    }
    ...
    public void forEachRemaining(final ShortConsumer action) {
        this.origin.forEachRemaining(obj -> action.accept(this.conversionFunction.applyAsShort(obj)));
    }
}
