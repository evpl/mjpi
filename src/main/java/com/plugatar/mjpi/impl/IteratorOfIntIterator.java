...
/**
 * The {@link Iterator} implementation based on {@link IntIterator}.
 *
 * @param <T> the type of Iterator items
 */
public final class IteratorOfIntIterator<T> implements Iterator<T> {
    private final IntIterator origin;
    private final IntFunction<? extends T> conversionFunction;

    /**
     * Ctor.
     *
     * @param origin             the origin iterator
     * @param conversionFunction the function to convert {@code int}-valued items to type {@code T}
     * @throws NullPointerException if {@code origin} or {@code conversionFunction} is null
     */
    public IteratorOfIntIterator(final IntIterator origin,
                                 final IntFunction<? extends T> conversionFunction) {
